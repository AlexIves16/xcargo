<template>
  <div class="rocket-widget">
    <div class="mini-artboard">
      <div class="stars-bg">
        <div class="shooting-star-mini" v-for="n in 5" :key="n"></div>
      </div>
      
      <div class="rocket-group">
        <div class="fire"></div>
        <svg id="rocket-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 154.1 259.1" enable-background="new 0 0 154.1 259.1">
          <component is="style">
...
<style scoped>
.rocket-widget {
  /* Reduced size approx 2x (from 120 to 60) */
  width: 60px;
  height: 60px;
  position: relative;
...
.stars-bg {
  ...
  width: 200%;
  height: 200%;
  transform: rotate(45deg); /* Align stars diagonally */
}
...
.rocket-group {
  ...
  top: 50%;
  left: 50%;
  width: 25px; /* Scaled down internal size */
  height: 40px;
  margin-left: -12.5px;
  margin-top: -20px;
  animation: hover-flight 3s ease-in-out infinite alternate;
...
/* Fire effects adapted - aligning with nozzle */
.fire {
  position: absolute;
  top: 88%; 
  left: -25%;
  ...
  transform: rotate(180deg); /* Trying close to vertical-diagonal */
  z-index: -1;
...
.shooting-star-mini {
  position: absolute;
  width: 1px;
  height: 10px; /* Scaled down tail */
  border-radius: 50%;
  background-color: #fff;
  opacity: 0;
  box-shadow: 0 0 2px 1px rgba(255, 255, 255, 0.15);
  animation: shooting-mini 4s infinite ease-in;
}

/* Manually position a few stars with varied delays */
.shooting-star-mini:nth-child(1) { left: 40%; top: -20%; animation-delay: 0s; }
.shooting-star-mini:nth-child(2) { left: 70%; top: -10%; animation-delay: 2.5s; }
.shooting-star-mini:nth-child(3) { left: 20%; top: 10%; animation-delay: 1.2s; }
.shooting-star-mini:nth-child(4) { left: 60%; top: -30%; animation-delay: 0.5s; }
.shooting-star-mini:nth-child(5) { left: 90%; top: 0%; animation-delay: 3.5s; }

@keyframes flicker {
  0% { transform: rotate(180deg) scale(0.8); box-shadow: 0 0 5px 2px rgba(129, 140, 248, 0.5); }
  50% { box-shadow: 0 0 5px 4px rgba(129, 140, 248, 0.5); }
  100% { box-shadow: 0 0 5px 2px rgba(129, 140, 248, 0.5); }
}
...
@keyframes shooting-mini {
  ...
  100% { 
    opacity: 0; 
    transform: translateY(100px); /* Fall straight down relative to rotated container */
  }
}
</style>
